...
class GLocationComponent1 : public GComponent<GLocationComponent1>, public sigslot::has_slots<>
{
public:
	GLocationComponent1() : GComponent(), mPosition(0.0f, 0.0f), mDefaultX(0.0f), mDefaultY(0.0f) {};
	GLocationComponent1(float x, float y) : mPosition(x, y), mDefaultX(x), mDefaultY(y), mLastX(x), mLastY(y)
	{
		mDefaultX = 10;
		bool d = true;
	};
	virtual ~GLocationComponent1()
	{
