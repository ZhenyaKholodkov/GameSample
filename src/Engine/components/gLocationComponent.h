...
public:
	...
	void setX(float x) 
	{
		signal_LocationChangedWithDxDy(x - mPosition.x, 0.0f);
		mLastX = mPosition.x; mPosition.x = x; 
	}
	void setY(float y) 
	{
		signal_LocationChangedWithDxDy(0.0f, y - mPosition.y);
		mLastY = mPosition.y; mPosition.y = y; 
	}
	void setXY(float x, float y)
	{
		signal_LocationChangedWithDxDy(x - mPosition.x, y - mPosition.y);
		mLastX = mPosition.x; mLastY = mPosition.y; mPosition.x = x; mPosition.y = y; 
	}
